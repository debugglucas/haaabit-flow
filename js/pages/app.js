    ...
    function init() {
        console.log("🚀 App Iniciado (V Final - Fixed)!");
    ...
    function setupEventListeners() {
            ...
            if (btnAction) {
                ...
                if (type === 'timer') {
                    ...
                    if (wasCompleted) {
                        ...
                        // 1. Zera o contador numérico (O básico)
                        if (!habit.progress) habit.progress = {};
                        habit.progress[today] = 0;
                        
                        // 2. Remove da lista de datas (Caso o habits.js use array de datas)
                        if (habit.completedDays && Array.isArray(habit.completedDays)) {
                            habit.completedDays = habit.completedDays.filter(d => d !== today);
                        }

                        // 3. Destrói qualquer propriedade "true" que esteja sobrando
                        habit.completed = false;
                        habit.done = false;
                        delete habit.completed; // Deleta a chave do objeto
                        delete habit.done;      // Deleta a chave do objeto
                        ...
                        // ... (Mantenha o bloco ELSE de completar igual estava) ...
        ...
        // Remove ouvintes antigos e adiciona novos
        if (pendingList) {
